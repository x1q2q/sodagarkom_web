...
<script type="text/javascript">
 document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('appCategory', () => ({
      ...
      openModalCategory(typeModal,item=null){
        if(typeModal == 'add'){
      ...
      async deleteCategory(){
        await fetch('<?= base_url(); ?>admin/categories/delete/'+this.dataEdit.id, {
              method: 'GET',
              headers: {
          ...
          .then((result) => {
            if(result.code == 200){
               this.closeModalCategory('delete');
      ...
      dataTable(){
        return {
          ...
          offset: 10,
          pagination: {
            total: this.dataCTG.length,
            lastPage: Math.ceil(this.dataCTG.length / 5),
            perPage: 5,
            currentPage: 1,
            from: 1,
            to: 1 * 5
          ...
          initData() {
            this.items = this.dataCTG.sort(this.compareOnKey('id', 'desc'))
            this.showPages()
          },
          ...
          search(value) {
            if (value.length > 1) {
              ...
              const fuse = new Fuse(this.dataCTG, options)
              this.items = fuse.search(value).map(elem => elem.item)
            } else {
              this.items = this.dataCTG
            }
            this.changePage(1)
            this.showPages()
          },
...
<main class="h-full overflow-y-auto" x-data="appCategory">
    <div
      x-cloak
        ...
        <div class="mt-4 mb-6">
          <p class="text-lg font-semibold text-gray-700 dark:text-gray-300">
            ...
            <label class="block text-sm mt-2">
              ...
              <input x-model="dataEdit.name" type="text" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="nama kategori" required>
            </label>
    ...
    <div
      ...
      x-transition:leave-start="opacity-100"
      x-transition:leave-end="opacity-0  transform translate-y-1/2"
      @click.away="closeModalCategory('delete')"
      class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl"
      role="dialog"
    ...
    >
      ...
      >
        <button
          @click="closeModalCategory('delete')"
...
<div x-cloak x-show="toastResult.isOpen" x-transition.duration.500m 
 ...
 style="right:2rem;top:5rem;width:25rem;">
  ...
  <div class="container px-6 mx-auto">
    <div class="flex justify-between items-center">
      ...
      <button @click="openModalCategory('add')"
            ...
            <tr x-show="checkView(index + 1)" class="text-gray-700 dark:text-gray-400">
              ...
              <td class="px-4 py-3">
                  <div class="flex items-center space-x-4 text-sm">
                    <button @click="openModalCategory('edit', item)"
                      class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                      aria-label="Edit"
                    >
                      <span class="text-2xl"><i class='bx bxs-edit-alt'></i></span>
                    </button>
                    <button @click="openModalCategory('delete',item.id)"
                      class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                      aria-label="Delete"
                    >
...
<div
  ...
  >
    ...
    <form
      ...
      x-transition:leave-start="opacity-100"
      x-transition:leave-end="opacity-0  transform translate-y-1/2"
      @click.away="closeModalCategory('add')"
      class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl"
      role="dialog"
    ...
    >
      <div class="mt-4 mb-6">
        ...
        <div class="mt-4">
          <label class="block text-sm mt-2">
            <span class="text-gray-700 dark:text-gray-400">Nama</span>
            <input name="name" type="text" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="nama kategori"
      ...
      <footer
        class="flex flex-col items-center justify-end px-6 py-3 -mx-6 -mb-4 space-y-4 sm:space-y-0 sm:space-x-6 sm:flex-row bg-gray-50 dark:bg-gray-800"
      >
        <button type="button" @click="closeModalCategory('add')"
          class="w-full px-5 py-3 text-sm font-medium leading-5 text-white text-gray-700 transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 sm:px-4 sm:py-2 sm:w-auto active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray"
        >
          Cancel
        </button>
